...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if u.effective_chat.type == 'private':
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ Group ထဲထည့်ရန်", url="https://t.me/myaigroup_ai_bot?startgroup=true")]])
...
async def track_members(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if u.chat_member.new_chat_members:
        ...
        await chat.send_message(f"✨ မင်္ဂလာပါ {user.first_name} ရေ... ကြိုဆိုပါတယ်ရှင့်cherryရှိအရပ်က။ 🥰")
...
async def main():
    ...
    async def handle_home(request):
        return web.Response(text="Web Server is Active!")
